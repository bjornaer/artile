...
from artile.core import process, trees
from artile.core.data import Tiled
from artile.core.iterators import Iterator
...
class Lifted:
    ...
    def init(self, *args, **kwargs):
        ...
        arg_indices = [
            arg_index
            for arg_index in trees.tree_scan(args)[2]
            if isinstance(trees.tree_index(args, arg_index), (Iterator, Tiled))
        ]
        kwarg_indices = [
            kwarg_index
            for kwarg_index in trees.tree_scan(kwargs)[2]
    ...
    def apply(self, processed_tiles, variables):
        ...
        if step < n_steps:
            ...
            if self.vectorized:
                ...
                processed_istree, processed_indices, processed_tiles = process.process_vectorized(
                    self.func,
                    self.batch_axis,
                    self.pad_final_batch,
                    self.batch_size,
                    args,
                    kwargs,
                    arg_indices,
