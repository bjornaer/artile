...
from artile.core import trees, utils
from artile.core.iterators import (
    BorderIndicesIterator,
    IndexIterator,
    StitchIndicesIterator,
    TileIndicesIterator,
)
from artile.core.jobs import Job
...
class Data(np.ndarray):
    ...
    """numpy.ndarray subclass for storing ArTile data."""
...
class Tiled(Data):
    ...
    """numpy.ndarray subclass for storing ArTile tiled data."""
    ...
    def __new__(cls, tiles, job, mask=None, isimage=True, stackable=False, tile_scales=None):
        ...
        tiles.mask = mask
        tiles.metadata = {"isimage": isimage, "stackable": stackable, "tile_scales": tile_scales}
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        ...
        from artile.core import process

        args = (ufunc, method, *inputs)
        arg_indices = [
            arg_index for arg_index in trees.tree_scan(args)[2] if isinstance(trees.tree_index(args, arg_index), Tiled)
        ]

        job_locals = {"args": trees.tree_apply(args, arg_indices, lambda ts: Tiled), "kwargs": kwargs}
        ...
        for index in zip(*nonempty_indices):
            processed_istree, processed_indices, processed_tiles = process.process_single(
                reference[index].__array_ufunc__,
                False,
                args,
                kwargs,
                arg_indices,
    ...
    def __array_function__(self, func, types, args, kwargs):
        ...
        job_locals = {"args": trees.tree_apply(args, arg_indices, lambda ts: Tiled), "kwargs": {}}
        ...
        for index in zip(*nonempty_indices):
            processed_istree, processed_indices, processed_tiles = process.process_single(
                lambda _func, _types, _args, _kwargs: reference[index].__array_function__(
                    _func, _types, tuple(_args), _kwargs
                ),
                False,
                args,
                {},
    ...
    def compute(self, batch_axis=False, pad_final_batch=False, batch_size=None, **kwargs):
        ...
        from artile.core.lift import lift

        if isinstance(self[self.nonempty_indices_tuples[0]], Array):
            tiles = lift(
                lambda tile: Output(tile.compute(**kwargs), **self.metadata),
    ...
    def persist(self, batch_axis=False, pad_final_batch=False, batch_size=None, **kwargs):
        ...
        if isinstance(self[self.nonempty_indices_tuples[0]], Array):
            tiles = lift(
                lambda tile: Output(tile.persist(**kwargs), **self.metadata),
                batch_axis=batch_axis,
                pad_final_batch=pad_final_batch,
                batch_size=batch_size,
            )(self)
    ...
    def pad(self, **kwargs):
        ...
        if self.metadata["isimage"] and not self.metadata["stackable"]:
            ...
            tile_padding = (
                tile_size[0] - (tile_indices[0][-1, 1] - tile_indices[0][-1, 0]),
                ...
                if (mode in ["reflect", "symmetric"]) and (tiles.shape[0] > 1):
                    ...
                    for i, tile in enumerate(tiles[-1]):
                        if tile is not None:
                            ...
                            tiles[-1, i] = utils.array_pad(
                                tile, tile_padding[0], -2, mode="constant", constant_values=constant
                            )
                else:
                    for i, tile in enumerate(tiles[-1]):
                        if tile is not None:
                    ...
                    for i, tile in enumerate(tiles[:, -1]):
                        if tile is not None:
                            ...
                            if mode == "tile_maximum":
                                ...
                                constant = np.min(tile)
                            tiles[i, -1] = utils.array_pad(
                                tile, tile_padding[1], -1, mode="constant", constant_values=constant
                            )
                else:
                    for i, tile in enumerate(tiles[:, -1]):
                        if tile is not None:
    ...
    def tile_indices(self):
        ...
        tile_indices = (
            np.rint(profile_tile_indices[0] * image_scales[0]).astype(int),
    ...
    def border_indices(self):
        ...
        border_indices = (
            np.rint(profile_border_indices[0] * image_scales[0]).astype(int),
...
class Stitched(Data):
    ...
    """numpy.ndarray subclass for storing ArTile stitched data."""
