...
from artile.core.data import Output, Tiled
...
def process_vectorized(
    ...
    batch_args, batch_kwargs = get_arguments(
        pad_final_batch, batch_size, args, kwargs, arg_indices, kwarg_indices, batch_indices
    )
    processed_batch_tiles = func(*batch_args, **batch_kwargs)

    processed_istree, processed_indices, processed_tiles = initialize_tree(
        processed_batch_tiles, job, reference, processed_istree, processed_indices, processed_tiles
    )
    ...
    for i_batch, index in enumerate(zip(*batch_indices)):
        ...
        if processed_istree:
            for processed_index in processed_indices:
                update_tiles(
                    trees.tree_index(processed_tiles, processed_index),
...
def process_single(
    ...
    processed_istree, processed_indices, processed_tiles = initialize_tree(
        processed_tile, job, reference, processed_istree, processed_indices, processed_tiles
    )
...
def get_arguments(pad_final_batch, batch_size, args, kwargs, arg_indices, kwarg_indices, batch_indices):
    ...
    partial_create_batch = partial(
        create_batch, pad_final_batch=pad_final_batch, batch_size=batch_size, batch_indices=batch_indices
    )
...
def initialize_tree(processed_tile, job, reference, processed_istree, processed_indices, processed_tiles):
    ...
    if processed_istree is None:
        ...
        processed_tiles = trees.tree_apply(
            processed_tile, processed_indices, partial(initialize_tiles, job=job, reference=reference)
        )
    ...
    for i, ts in enumerate(tiles):
        if profile is None:
            ...
            if ts.profile is not profile:
                raise ValueError("tiles must all share a common profile.")

        if mask is None:
            mask = ts.mask
        else:
            if np.any(ts.mask != mask):
                raise ValueError("tiles must all share a common mask.")
