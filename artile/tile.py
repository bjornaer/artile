from artile.core import utils
from artile.core.data import Tiled
from artile.core.jobs import Job
from artile.core.profiles import Profile
...
class ArTileArray(ArTile):

    """ArTile subclass for arrays.
...
class ArTileFunction(ArTile):

    """ArTile subclass for functions.
    ...
    def get_tiles(self, tile_size, overlap=(0.1, 0.1), slices=(slice(None),)):
        ...
        from artile.sources import function
...
class ArTileLargeImage(ArTile):

    """ArTile subclass for large_image tile sources.
    ...
    def get_tiles(self, tile_size, overlap=(0.1, 0.1), slices=0):
        ...
        from artile.sources import large_image

        self.image_shape = (self.image.getMetadata()["sizeY"], self.image.getMetadata()["sizeX"])
        tiles, tiling, tile_indices, border_indices = large_image.parse(
            self.image, self.image_shape, tile_size, overlap, slices
        )
        nonempty_indices = utils.get_nonempty_indices(tiles)
...
class ArTileND2(ArTile):

    """ArTile subclass for ND2 files.
    ...
    def get_tiles(self, overlap=(0.1, 0.1), slices=(slice(None))):
        ...
        tiles, tiling, tile_size, overlap, self.image_shape = nd2.parse(
            self.image, self.dask, self.axis_sizes, self.axis_order, overlap, slices
        )
