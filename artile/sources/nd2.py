...
def parse(image, dask, axis_sizes, axis_order, overlap, slices):
    with nd2.ND2File(image) as image:
        ...
        image_array = np.moveaxis(
            image_array, range(len(raw_axis_order)), (axis_order.index(axis) for axis in raw_axis_order)
        )
        ...
        if "P" not in axis_order:
            ...
            if overlap is None:
                ...
                x_overlap = round(
                    1
                    - (np.mean(np.diff(x_overlaps)) / image.metadata.channels[0].volume.axesCalibration[0])
                    / image.attributes.widthPx,
                    2,
                )
                y_overlap = round(
                    1
                    - (np.mean(np.diff(y_overlaps)) / image.metadata.channels[0].volume.axesCalibration[1])
                    / image.attributes.heightPx,
                    2,
                )
