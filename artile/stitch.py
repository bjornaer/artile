...
from artile.core.data import Stitched
...
def stitch_image(tiles, blend=True, sigma=10):
    ...
    if blend:
        ...
        for idx in nonempty_indices:
            tile_index = tile_indices_iterator[idx]

            tile = tiles[idx]
            stitch_slice = np.s_[..., tile_index[0, 0] : tile_index[0, 1], tile_index[1, 0] : tile_index[1, 1]]
            tile_slice = np.s_[..., : tile_index[0, 1] - tile_index[0, 0], : tile_index[1, 1] - tile_index[1, 0]]
        ...
        for idx in nonempty_indices:
            i_image, j_image, i_tile, j_tile = stitch_indices_iterator[idx]
...
def stitch_masks(masks, iou_threshold=0.1):
    ...
    for z in range(mask_flat_shape[0]):
        ...
        for idx, blobs in border_blobs.items():
            tile_index = tile_indices_iterator[idx]
            ...
            for blob in blobs:
                ...
                s_image = (
                    slice(s[0].start + tile_index[0, 0], s[0].stop + tile_index[0, 0]),
                    slice(s[1].start + tile_index[1, 0], s[1].stop + tile_index[1, 0]),
                )
...
def stitch_coords(coords):
    ...
    for n in range(n_batches):
        ...
        for idx in nonempty_indices:
            tile_index = tile_indices_iterator[idx]
            border_index = border_indices_iterator[idx] - 0.5

            if batch_axis:
                coord = coords[idx][n].copy()
            else:
                coord = coords[idx].copy()

            coord[:, -2:] = coord[:, -2:] + np.array([tile_index[0, 0], tile_index[1, 0]])
            s = (
                (border_index[0, 0] < coord[:, -2])
                & (coord[:, -2] < border_index[0, 1])
                & (border_index[1, 0] < coord[:, -1])
                & (coord[:, -1] < border_index[1, 1])
            )
